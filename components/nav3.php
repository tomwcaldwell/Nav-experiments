<div class="nav-3">
	<div class="w-full absolute z-20">

		<div class="navigation-3 nav-detail-3 bg-green-light w-full">
			<div class="w-3/4 mx-auto h-full">
				<ul class="list-reset hidden flex justify-between items-center h-full">
					
					<div class="w-1/4">
						<div class="nav-3-image-1 w-2/3 mx-auto h-80 flex items-center justify-center profile image-li cursor-pointer">
							<div class="hover-container-1 flex items-center justify-center">
								<li class="hover-title-1 text-5.5xl text-white opacity-0">Profile</li>
							</div>
						</div>
					</div>

					<div class="w-1/4">
						<div class="nav-3-image-2 w-2/3 mx-auto h-80 flex items-center justify-center work image-li cursor-pointer">
							<div class="hover-container-2 flex items-center justify-center">
								<li class="hover-title-2 text-5.5xl text-white opacity-0">Work</li>
							</div>
						</div>
					</div>

					<div class="w-1/4">
						<div class="nav-3-image-3 w-2/3 mx-auto h-80 flex items-center justify-center insights image-li cursor-pointer">
							<div class="hover-container-3 flex items-center justify-center">
								<li class="hover-title-3 text-5.5xl text-white opacity-0">Insights</li>
							</div>
						</div>
					</div>

					<div class="w-1/4">
						<div class="nav-3-image-4 w-2/3 mx-auto h-80 flex items-center justify-center talk image-li cursor-pointer">
							<div class="hover-container-4 flex items-center justify-center">
								<li class="hover-title-4 text-5.5xl text-white opacity-0">Talk</li>
							</div>
						</div>
					</div>

				</ul>
				<div class="relative">
					
					<div class="nav-contact-3 hidden">
						<div class="font-atsurt text-white flex justify-between absolute pin-b mb-20 w-full">
							<div class="w-1/2 pl-10 text-sm text-left leading-normal opacity-50">
								12 Southgate Street<br />
								Winchester, SO23 9EF
							</div>
							
							<div class="w-1/2 pr-10 text-sm text-right leading-normal opacity-50">
								<span class="cursor-pointer">Instagram</span><br />
								<span class="cursor-pointer">Twitter</span>
							</div>
						</div>
					</div>

				</div>
			</div>
			<div class="nav-bottom-3 nav-bottom-3--start bg-white py-1"></div>
		</div>
		
	</div>
</div>